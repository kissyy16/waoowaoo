...
describe('model-gateway openai-compat responses executor', () => {
  ...
  it('converts responses payload to normalized chat completion', async () => {
    const fetchMock = vi.fn(async () => new Response(JSON.stringify({
      output: [
        { type: 'reasoning', text: 'think-', summary: [{ type: 'summary_text', text: 'summary-' }] },
    ...
    expect(completion.choices[0]?.message?.content).toEqual([
      { type: 'reasoning', text: 'think-summary-' },
  ...
  it('omits responses reasoning options when reasoning is disabled', async () => {
    const fetchMock = vi.fn(async () => new Response(JSON.stringify({
      output_text: 'hello',
    }), { status: 200 }))
    vi.stubGlobal('fetch', fetchMock)
