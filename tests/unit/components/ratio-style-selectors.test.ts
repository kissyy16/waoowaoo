...
describe('RatioStyleSelectors', () => {
  ...
  it('renders ratio, style, duration, and style preset dropdown panels through a portal to document.body', () => {
    const fakeDocument = {
      body: { nodeName: 'BODY' },
    }
    ...
    const html = renderToStaticMarkup(
      createElement('div', null,
        ...
        createElement(DurationSelector, {
          value: 30,
          onChange: () => undefined,
        }),
    ...
    expect(portalMocks.createPortalMock).toHaveBeenCalledTimes(4)
    ...
    expect(portalMocks.createPortalMock.mock.calls[3]?.[1]).toBe(fakeDocument.body)
